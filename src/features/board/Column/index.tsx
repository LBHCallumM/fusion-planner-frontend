...
interface Props {
  ...
  boardId: string;
  handleAddNewCard: (newCard: ICard, columnId: string) => void;
}
const Column = ({ column, cards, boardId, handleAddNewCard }: Props) => {
            ...
            isAddingCard ? (
              <div className="mt-2 ">
                <form onSubmit={handleAddCard}>
                  <TextareaAutosize name="" id="" cols={30} rows={10} className="bg-gray-50 w-full p-2 flex justify-between rounded-sm text-gray-700 shadow-sm hover:bg-gray-100 border-gray-300 group" value={newCardName} onInput={e => setNewCardName(e.target.value)} />


                  <div className="mt-2 flex items-center">
                    <button className=" bg-gray-600 rounded-sm px-3 text-white mr-2" type="submit">Add card</button>
                    <button onClick={handleCancelCardEditor} type="button"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
